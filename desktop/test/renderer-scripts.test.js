/*
 * The renderer's scripts, loaded the way the browser loads them.
 *
 * Each page pulls in several classic scripts, and classic scripts share one
 * global lexical scope: a top-level `const` in the second file collides with a
 * top-level `function` of the same name in the first, and the collision is a
 * SyntaxError raised at parse time, before a single statement runs. The whole
 * page dies — no pets, no cards, no click-to-jump — and nothing else in the
 * suite can see it, because every other test `require()`s these files and
 * CommonJS gives each its own module scope.
 *
 * This shipped once. Each page's script list is read out of its HTML rather than
 * written down here, so adding a script to a page brings it under test too.
 */
const test = require('node:test');
const assert = require('node:assert');
const vm = require('node:vm');
const fs = require('node:fs');
const path = require('node:path');

const DESKTOP = path.join(__dirname, '..');

/*
 * Every page in the app that loads classic scripts, with the preload whose
 * surface its scripts are allowed to call. Two preloads rather than one is
 * deliberate — the Pets window has no business being able to claim the pointer
 * or answer an approval — so the pairing has to be checked per page.
 */
const PAGES = [
  { html: 'overlay.html', preload: 'preload.js', bridge: 'petsBridge' },
  { html: 'pets-window.html', preload: 'pets-preload.js', bridge: 'straysPets' },
];

/* the scripts a page loads, in the order it loads them */
function pageScripts(htmlFile) {
  const html = fs.readFileSync(path.join(DESKTOP, htmlFile), 'utf8');
  return [...html.matchAll(/<script\s+src="([^"]+)"/g)].map((m) => ({
    src: m[1],
    code: fs.readFileSync(path.resolve(DESKTOP, m[1]), 'utf8'),
  }));
}

/* just enough browser for the scripts to reach the end of their top level */
function browserGlobals(bridgeName) {
  const noop = () => {};
  const element = () => ({
    className: '', textContent: '', style: {}, children: [],
    width: 0, height: 0, clientWidth: 1440, clientHeight: 190, offsetWidth: 360,
    dataset: {}, checked: false, title: '', type: '', draggable: false,
    classList: { add: noop, remove: noop, toggle: noop, contains: () => false },
    setAttribute: noop, removeAttribute: noop, remove: noop,
    addEventListener: noop, removeEventListener: noop,
    appendChild: (c) => c,
    getBoundingClientRect: () => ({ left: 0, top: 0, width: 1440, height: 190 }),
    getContext: () => new Proxy({}, {
      get: (_t, k) => (k === 'measureText' ? () => ({ width: 10 })
        : k === 'createRadialGradient' ? () => ({ addColorStop: noop })
          : noop),
    }),
  });

  const globals = {
    console: { log: noop, warn: noop, error: noop },
    document: {
      hidden: false, currentScript: null, body: element(),
      createElement: element, getElementById: () => element(),
      querySelector: () => null, querySelectorAll: () => [],
      addEventListener: noop, removeEventListener: noop,
    },
    localStorage: { getItem: () => null, setItem: noop },
    requestAnimationFrame: () => 0,
    cancelAnimationFrame: noop,
    setTimeout: () => 0,
    clearTimeout: noop,
    setInterval: () => 0,
    clearInterval: noop,
    Image: function Image() { return element(); },
  };
  globals.window = globals;
  globals.globalThis = globals;
  globals.window.innerWidth = 1440;
  globals.window.innerHeight = 900;
  globals.window.devicePixelRatio = 1;
  globals.window.addEventListener = noop;
  globals.window.removeEventListener = noop;

  /*
   * The preload's contextBridge surface. Answering every name with a noop keeps
   * this stub from being a second list to maintain — listing the methods by hand
   * meant that adding one to the renderer failed here, in a test whose whole job
   * is to catch a different bug entirely. Whether the preload really exposes
   * what the renderer subscribes to is checked directly, further down.
   *
   * The one shape that cannot be a bare noop is a call the renderer awaits and
   * then reads fields off: `pets:load` answers with lists, and a noop return
   * makes the page throw on `.map` for reasons that have nothing to do with the
   * collision this file exists to catch.
   */
  const answer = () => Promise.resolve({ pets: [], enabled: [], toggles: {} });
  globals.window[bridgeName] = new Proxy({}, { get: () => answer });
  return globals;
}

for (const page of PAGES) {
  test(`every script ${page.html} loads can be loaded together`, () => {
    const scripts = pageScripts(page.html);
    assert.ok(scripts.length >= 2, `${page.html} should load more than one script`);

    const context = vm.createContext(browserGlobals(page.bridge));
    const loaded = [];
    for (const { src, code } of scripts) {
      try {
        vm.runInContext(code, context, { filename: src });
      } catch (err) {
        assert.fail(
          `${src} failed to load after [${loaded.join(', ')}]: ${err.name}: ${err.message}`,
        );
      }
      loaded.push(src);
    }
  });

  test(`${page.preload} exposes every bridge method ${page.html} subscribes to`, () => {
    /*
     * The renderer reaches the main process only through the preload's
     * contextBridge surface. A listener the renderer registers but the preload
     * never exposes throws on the line that registers it, which — in a classic
     * script — abandons the rest of the file. Adding a channel means touching two
     * files, so the pairing is asserted rather than assumed.
     */
    const preload = fs.readFileSync(path.join(DESKTOP, page.preload), 'utf8');
    const exposed = new Set(
      [...preload.matchAll(/^\s{2}([A-Za-z_$][\w$]*)\s*:/gm)].map((m) => m[1]),
    );
    assert.ok(exposed.size > 2, `the ${page.preload} surface should have been found`);

    const calls = new RegExp(`${page.bridge}\\.([A-Za-z_$][\\w$]*)`, 'g');
    const used = new Set(
      pageScripts(page.html).flatMap(({ code }) =>
        [...code.matchAll(calls)].map((m) => m[1])),
    );
    assert.ok(used.size > 0, `${page.html} should use the bridge`);

    for (const name of used) {
      assert.ok(exposed.has(name),
        `the renderer calls ${page.bridge}.${name}, which ${page.preload} does not expose`);
    }
  });

  test(`no two scripts ${page.html} loads declare the same global name`, () => {
    // The specific collision that killed the renderer, stated directly: a script
    // that only wants a helper from an earlier one must not re-declare its name.
    const seen = new Map();
    for (const { src, code } of pageScripts(page.html)) {
      const declarations = [...code.matchAll(/^(?:const|let|var|function)\s+([A-Za-z_$][\w$]*)/gm)]
        .map((m) => m[1])
        .concat([...code.matchAll(/^const\s*\{([^}]+)\}/gm)]
          .flatMap((m) => m[1].split(',').map((s) => s.split(':').pop().trim()))
          .filter(Boolean));

      for (const name of declarations) {
        const owner = seen.get(name);
        assert.ok(
          owner === undefined || owner === src,
          `${src} re-declares "${name}", already declared by ${owner} — ` +
          'classic scripts share one global scope, so this is a parse-time SyntaxError',
        );
        seen.set(name, src);
      }
    }
  });
}

/*
 * The two preloads are deliberately different surfaces. The Pets window is an
 * ordinary focusable window; handing it the pointer lease or the approval reply
 * channel would mean a settings panel could take every click on the machine.
 */
test('the Pets window preload cannot reach the overlay-only channels', () => {
  const petsPreload = fs.readFileSync(path.join(DESKTOP, 'pets-preload.js'), 'utf8');
  for (const forbidden of ['setInteractive', 'approvalReply', 'jumpToSession']) {
    assert.ok(!petsPreload.includes(forbidden),
      `pets-preload.js exposes ${forbidden}, which belongs to the lane alone`);
  }
});
